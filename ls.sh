...
udocker pull fedora
udocker create --name=fedora fedora
udocker setup --execmode=R1 fedora

# Create the script to be executed inside the container
cat > fedora_setup.sh << EOF
#!/bin/sh
cd && dnf update -y
dnf install -y tigervnc-server xterm dbus-x11 mesa-dri-drivers @xfce-desktop-environment
...
udocker run -v `pwd`:/mnt fedora /bin/sh /mnt/fedora_setup.sh 
...
export XDG_RUNTIME_DIR=$HOME
export PATH=`pwd`/udocker-1.3.17/udocker:$PATH
udocker run fedora /bin/sh /startup.sh
udocker setup --execmode=R1 fedora
EOF
